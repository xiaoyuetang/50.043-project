...
from app import db, log, meta, mysql
...
def insert_review(id,asin,helpful,overall,reviewText,reviewTime,reviewerID,reviewerName,summary,unixReviewTime):
	...
	try:
		...
		return("Record inserted successfully into reviews table")
...
def get_last_id():
	try:
		conn = mysql.connection.cursor()
		conn.execute('SELECT MAX(id) FROM reviews')
		last_id = conn.fetchone()
		return last_id
	except Error as error:
	    print(error)
...
def submit_review():
	...
	if 'reviewbutton' in submittedreview:
		...
		reviewTime = '-1'
		reviewerID = '-1'
		reviewerName = 'jenyang'
		summary = summary
		unixReviewTime = '-2'
		result = insert_review(eyed,asin,helpful,overall,reviewText,reviewTime,reviewerID,reviewerName,summary,unixReviewTime)
